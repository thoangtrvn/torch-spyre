...
def SimpleIterate(device=None):
    ...
    var0 = torch.rand(torch.Size([48]), dtype=torch.float32).to(device)
    var1 = torch.rand(torch.Size([2, 1, 11, 48]), dtype=torch.float32).to(device)
    inputs = [var0, var1]
...
@pytest.mark.largedimtensor
@pytest.mark.fpoperation
def test_mul_49():
