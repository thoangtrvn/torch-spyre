...
c10::intrusive_ptr<Work> SpyreCCLBackend::allreduce(
  ...
  try {
    ...
    // NOTE: >=2-D tensors take the SAME native LIBCOLL Ring path as 1-D
    // (enqueue_async -> ctx.allreduce, which flattens to {numel} and
    // reduce-scatters). This is HW-verified correct for 2-D (even/uneven/decode/
    // prefill + subgroup) once torch.ones/full produce the normal stickified
    // layout -- the earlier "2-D under-reduce" was that transposed-ones layout
    // bug (fixed in ops/eager.py), NOT the Ring split geometry. No special 2-D
    // compose path is needed.
