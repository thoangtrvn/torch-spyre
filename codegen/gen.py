...
def generate_and_register_wrappers(codegen_dir: Union[str, Path]):
    """
    Generates the Python file that implements wrapper functions and handles registrations for the dispatcher.
    ...
    # NOTE: We have an existing Python file (spyre_torch_ops.py), which will be extended with the functions
    ...
    body_content_parts = [
        "\n".join(body_content_lines[:placeholder_idx]),
        "\n".join(body_content_lines[placeholder_idx + 1:]),
    ]

    header = body_content_parts[0]
    header_lines = header.split('\n')
    # TO-DO: Build the import statement dynamically
    new_imports = 'from typing import Union, Optional, Any, List'
    target_index = None
    ...
    if target_index is not None:
        header_lines.insert(target_index + 1, new_imports)
    header = "\n".join(header_lines)
    body_content_parts[0] = header

    # Add warning message after imports but before placeholder
    py_output = body_content_parts[0] + "\n\n" + warning_msg + "\n" + first_comment_text
    ...
    for rep_data in replacements:
        ...
        py_output += generate_from_template(templates_path, template_name, rep_data)
        supported_ops.append(rep_data)

    py_output += second_comment_text + body_content_parts[1]

    outputs_path.mkdir(exist_ok=True)
    output_file_path =  codegen_path.parent / "torch_spyre"  / "codegen_ops.py"
    ...
    print(
        f"Successfully generated {len(supported_ops)} operations and updated to {output_file_path}."
    )
    return outputs_path
